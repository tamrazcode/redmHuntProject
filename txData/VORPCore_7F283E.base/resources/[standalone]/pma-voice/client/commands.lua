...
function setProximityState(proximityRange, isCustom)
	...
	MumbleSetTalkerProximity(GetPmaVoiceReceiveRange())
...
-- Увеличить дальность войса (для кнопки +)
RegisterCommand('cycleproximityup', function()
	if disableProximityCycle then return end
	local newMode = mode + 1
	if newMode <= #Cfg.voiceModes then
		mode = newMode
		...
		-- Достигнут максимум (Крик) — повторно отправляем событие для подсветки обруча и худа
		TriggerEvent('pma-voice:setTalkingMode', mode)
	end
end, false)

-- Уменьшить дальность войса (для кнопки -)
RegisterCommand('cycleproximitydown', function()
	...
	if newMode >= 1 then
		...
		-- Достигнут минимум (Шёпот) — повторно отправляем событие для подсветки обруча и худа
		TriggerEvent('pma-voice:setTalkingMode', mode)
	end
end, false)

if RegisterKeyMapping then
	pcall(RegisterKeyMapping, 'cycleproximity', 'Cycle Proximity', 'keyboard', GetConvar('voice_defaultCycle', 'NONE'))
	pcall(RegisterKeyMapping, 'cycleproximityup', 'Увеличить дальность войса', 'keyboard', 'UP')
	pcall(RegisterKeyMapping, 'cycleproximitydown', 'Уменьшить дальность войса', 'keyboard', 'DOWN')
end
