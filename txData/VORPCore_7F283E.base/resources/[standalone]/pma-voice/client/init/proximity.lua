...
function orig_addProximityCheck(ply)
	...
	local selectedProximity = LocalPlayer.state.proximity
	local voiceRange = type(selectedProximity) == 'table' and tonumber(selectedProximity.distance) or nil
	voiceRange = voiceRange or proximity

	if GetConvar('voice_useNativeAudio', 'false') == 'true' then
		voiceRange = voiceRange * 3
	end
