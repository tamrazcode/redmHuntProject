...
CreateThread(function()
    -- HUNT owns the inventory and weapon-item state.  The legacy ammo
    -- registration below targets vorp_inventory, which is intentionally not
    -- started in this profile; HUNT item handlers remain authoritative.
    if GetConvar('hunt_inventory_owner', '0') == '1' then
        print('^3[vorp_weaponsv2] HUNT inventory owner enabled; legacy ammo item hooks skipped.^7')
        return
    end
    if GetResourceState('vorp_inventory') ~= 'started' then
        print('^3[vorp_weaponsv2] vorp_inventory is unavailable; legacy ammo item hooks skipped.^7')
        return
    end
