...
local HuntInventoryOwner <const> = GetConvar('hunt_inventory_owner', '0') == '1'
...
local function registerStorage(prefix, name, limit)
    if HuntInventoryOwner then return false end
...
RegisterNetEvent("vorp_medic:Server:OpenStorage", function(key)
    ...
    if HuntInventoryOwner then
        return Core.NotifyObjective(_source, 'Медицинское хранилище доступно после интеграции с HUNT-инвентарём.', 5000)
    end
...
AddEventHandler("onResourceStop", function(resource)
    ...
    for key, _ in pairs(Config.Storage) do
        ...
        if not HuntInventoryOwner and GetResourceState('vorp_inventory') == 'started' then
...
AddEventHandler("onResourceStart", function(resource)
    ...
    if not HuntInventoryOwner then
...
CreateThread(function()
    ...
    if HuntInventoryOwner or GetResourceState('vorp_inventory') ~= 'started' then return end
