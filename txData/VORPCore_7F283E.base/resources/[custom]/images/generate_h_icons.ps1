...
foreach ($f in $files) {
    ...
    if ($len -eq 556693) {
        $targetName = "burdock_leaf_h.png" # Лист лопуха
    } elseif ($len -eq 223118) {
        $targetName = "twigs_h.png"        # Ветка
    } elseif ($len -eq 591808) {
        $targetName = "cloth_h.png"        # Ткань
    } elseif ($len -eq 447725) {
        $targetName = "bandage_burdock_h.png" # Повязка с лопухом
    }
    ...
    if ($targetName) {
        ...
        $craftPath = [System.IO.Path]::Combine("c:\Users\borch\Desktop\RedM_Server\txData\VORPCore_7F283E.base\resources\[custom]\thehunt_crafting\html\images", $targetName)
        $invPath = [System.IO.Path]::Combine("c:\Users\borch\Desktop\RedM_Server\txData\VORPCore_7F283E.base\resources\[custom]\thehunt_inventory\html\images", $targetName)

        $cropped.Save($craftPath, [System.Drawing.Imaging.ImageFormat]::Png)
        $cropped.Save($invPath, [System.Drawing.Imaging.ImageFormat]::Png)

        [Console]::WriteLine("SUCCESS: Generated $targetName ($($cropped.Width)x$($cropped.Height))")
