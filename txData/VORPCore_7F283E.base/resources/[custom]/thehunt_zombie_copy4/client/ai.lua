...
local function transition(r, state, now)
    ...
    if ped and DoesEntityExist(ped) then
        ...
        if not (wasCombat and isCombat) then
            ...
            -- the ambient idle graph and can leave every fresh zombie frozen.
            ClearPedTasks(ped)
        end
    end
end
...
states.CHASE = function(r, now)
    ...
    if inAttackRange then
        ...
        if not r.attackModeActive then
            ...
            -- Блокируем события и запрещаем панику/бегство, чтобы зомби никогда не убегали в страхе
        ...
        -- Direct melee strike enforcement: if close and not already throwing a strike, prompt direct melee engagement
...
function ZC.think(r, now)
    ...
    if not ped or not DoesEntityExist(ped) or r.dead or IsEntityDead(ped) then
        if ped and DoesEntityExist(ped) and not r.dead and IsEntityDead(ped) and NetworkHasControlOfEntity(ped) and now >= (r.deathReportAt or 0) then
