...
function ZC.canPlayZombieSound(stage, nowTime)
    ...
    local now = nowTime or (type(GetGameTimer) == 'function' and GetGameTimer() or 0)
    local maxLimit = (ZombieConfig and ZombieConfig.MaxAliveSounds) or 2
    local minGap = (ZombieConfig and ZombieConfig.AliveSoundMinGap) or 1500

    if ZC.getActiveAliveSoundCount(now) >= maxLimit then
        return false
    end

    if (now - (ZC.lastAliveSoundStartedAt or 0)) < minGap then
        return false
    end
