...
assert(Zombie.validate(test).walkStyle=='MP_Style_drunk', 'invalid walkStyle fallback failed')
 ...
 test.walkStyle='MP_Style_Casual'
 assert(Zombie.validate(test).walkStyle=='MP_Style_drunk', 'human walk style was accepted for zombie')
...
assert(ZS.count()==2,'duplicate commit')
 -- NPC damage must remain fully native: no client/server proximity-hit endpoint exists.
 assert(handlers['thehunt_zombie:attack']==nil,'scripted zombie damage handler is still registered')
 assert(eventCount('thehunt_zombie:hit')==0,'scripted zombie damage event was emitted')
 -- Network ownership transfer keeps the same entity, HP and identity.
 entities[101].hp=227;owners[101]=2
 assert(entities[101].hp==227)
 exported.SetPlayerAggression(1,false)
 assert(Player(1).state.huntZombieImmune)
...
print('      native-damage isolation, immunity, death once, reentry cooldown, late commit, permissions, revisions, buckets, disconnect, delete')
...
assert(r.state=='CHASE' and r.target==1,'brief LOS loss reset pursuit')
ZC.think(r,4600)
...
ZC.immune[1]=true;los=true;p.pos.x=10;assert(not ZC.visible(r,p))
 ...
 -- A newly replicated zombie starts as WANDER and must receive a route on the
 -- first owner tick, rather than being overwritten to an infinite stand task.
 ZC.players={}
 local spawned={id='spawned',ped=11,settings=Zombie.settings(z),variance=1,owned=false,state='WANDER',since=0,
  home={x=0,y=0,z=0},radius=40,idleTime=9000}
 ZC.peds.spawned=spawned;ZC.think(spawned,30000)
 assert(spawned.state=='WANDER' and spawned.roamGoal and lastMove,'fresh zombie did not begin wandering')
 ''')
print('PASS: production perception/FSM: LOS, field of view, crouch, reaction, lost target, immunity, migration resume')

# Damage is now exclusively the engine's native combat impact. No proxy hit path
# may remain capable of changing the victim's health merely from proximity.
manifest = (ROOT/'fxmanifest.lua').read_text(encoding='utf-8')
runtime_source = (ROOT/'client/runtime.lua').read_text(encoding='utf-8')
ai_source = (ROOT/'client/ai.lua').read_text(encoding='utf-8')
server_source = (ROOT/'server/main.lua').read_text(encoding='utf-8')
assert "'client/contact.lua'" not in manifest
assert 'thehunt_zombie:attack' not in runtime_source + ai_source + server_source
assert 'thehunt_zombie:hit' not in runtime_source + ai_source + server_source
assert 'SetEntityHealth(me' not in runtime_source
assert 'ApplyDamageToPed(me' not in runtime_source
assert 'TaskPutPedDirectlyIntoMelee' in ai_source
assert 'ZC.meleeAttackers' not in ai_source
assert '0.0, -1.0, 0.0, false' in ai_source
assert 'ZC.contact' not in ai_source
assert 'ambientSound' in ai_source and 'idleMotion' in ai_source
assert 'noticeZombieCorpse' in ai_source and 'r.meleeTaskAt = now + 2000' in ai_source
assert 'candidate, false, -1' in runtime_source and 'walk, true, -1' in runtime_source
assert '0x923583741DC87BCE' in runtime_source and '0x89F5E7ADECCCB49C' in runtime_source
assert 'SetPedCombatAbility(ped, 3)' in runtime_source and 'SetPedMeleeCombatLimits' not in runtime_source
assert 'zombie_very_drunk' in runtime_source
assert 'SetPedCanPlayAmbientIdles' not in ai_source
detect_source = (ROOT/'client/detection.lua').read_text(encoding='utf-8')
editor_source = (ROOT/'client/editor.lua').read_text(encoding='utf-8')
assert 'weatherSenseMultiplier' in detect_source and "r.settings.aggression > 0" not in detect_source.split("RegisterNetEvent('thehunt_zombie:noise'", 1)[1]
assert "'thehunt_gizmo'" in manifest and "RegisterNUICallback('gizmo'" in editor_source
assert 'local rightMouse=IsDisabledControlPressed(0,0xF84FA74F)' in editor_source
assert 'IsDisabledControlPressed(0,0x07CE1E61)' not in editor_source and 'drawZoneVolume' in editor_source
config_source = (ROOT/'config.lua').read_text(encoding='utf-8')
assert 'MaxAlive = 500' in config_source and 'MaxEntities = 1000' in config_source and 'CorpseLifetime = 90000' in config_source
assert 'MaxPerZone' not in config_source
print('PASS: only native NPC combat can damage players; idle motion and sparse non-verbal ambience are enabled')
