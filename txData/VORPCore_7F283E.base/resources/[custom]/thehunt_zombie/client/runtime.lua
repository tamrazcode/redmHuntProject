...
function ZC.applyWalkStyle(ped, style)
    ...
    -- MP layers alone are not consistently visible on ambient/networked NPCs.
    -- Apply the RedM locomotion graph too, keeping combat itself native.
    local locomotion = {
        MP_Style_drunk      = { base = 'default',       motion = 'moderate_drunk' },
        zombie_very_drunk   = { base = 'default',       motion = 'very_drunk' },
        MP_Style_Crazy      = { base = 'murfree',       motion = 'agitated' },
        MP_Style_SilentType = { base = 'default',       motion = 'stealth' },
        MP_Style_EasyRider  = { base = 'lone_prisoner', motion = 'normal' },
        MP_Style_Veteran    = { base = 'war_veteran',   motion = 'injured_general' },
        MP_Style_Greenhorn  = { base = 'lost_Man',      motion = 'lost_man_normal' },
    }
    local zombieWalks = {
        'MP_Style_drunk', 'MP_Style_Crazy', 'MP_Style_SilentType',
        'MP_Style_EasyRider', 'MP_Style_Veteran', 'MP_Style_Greenhorn',
        'zombie_very_drunk'
    }
    pcall(function()
        -- This native layers styles. Remove the former layer first, otherwise an
        -- NPC can keep its spawn/default locomotion and the newly selected style
        -- has no visible effect.
        for _, candidate in ipairs(zombieWalks) do
            Citizen.InvokeNative(0xCB9401F918CB0F75, ped, candidate, false, -1)
        end
        if walk ~= 'none' and walk ~= 'default' then
            Citizen.InvokeNative(0xCB9401F918CB0F75, ped, walk, true, -1)
        end
        local chosen = (walk == 'default' and { base = 'default', motion = 'default' }) or locomotion[walk] or locomotion.MP_Style_drunk
        Citizen.InvokeNative(0x923583741DC87BCE, ped, chosen.base)
        Citizen.InvokeNative(0x89F5E7ADECCCB49C, ped, chosen.motion)
    end)
end
function ZC.configure(ped,settings,initial,weapon)
    ...
    SetPedCombatAttributes(ped, 50, true) -- CA_CAN_CHARGE: charge straight in, don't flank
    SetPedCombatAttributes(ped, 58, true) -- CA_DISABLE_FLEE_FROM_COMBAT
    SetPedCombatAttributes(ped, 0, false) -- BF_CanUseCover = false
    SetPedCombatAttributes(ped, 1, true)  -- CA_USE_VEHICLE = true (Allows interaction with vehicles and mounts)
    SetPedCombatAttributes(ped, 2, false) -- without drivebys
    SetPedCombatAttributes(ped, 4, false) -- CA_STRAFE_BASED_ON_TARGET_PROXIMITY = false (no shuffling on the spot)
    ...
    SetPedCombatAttributes(ped, 22, true) -- CA_CAN_DRAG_PED_FROM_CAR = true (Allows unseating riders from mounts)
    SetPedCombatAttributes(ped, 26, true) -- CA_DISABLE_ENTRY_REACTIONS: never flinch or panic on combat initiation
    ...
    SetPedCombatAttributes(ped, 35, true) -- CA_DISABLE_PINNED_DOWN: never cower under fire
    SetPedCombatAttributes(ped, 38, true) -- CA_DISABLE_BULLET_REACTIONS: never flee from gunshots
    ...
    local agg = tonumber((settings and settings.aggression) or 1.0) or 1.0
    if agg >= 4 then
        SetPedCombatMovement(ped, 3)           -- Suicidal offensive (relentless charge, stage 4-5)
    else
        SetPedCombatMovement(ped, 2)           -- Aggressive advance (WillAdvance)
    end
    SetPedCombatRange(ped, 0)              -- Melee range
    -- RDR combat ability is an enum from 0 to 3.  Passing 100 does not mean
    -- "maximum"; it produces an invalid combat profile and can leave a ped
    -- facing its target without choosing a strike.  3 is suicidal/offensive.
    SetPedCombatAbility(ped, 3)
    SetPedFleeAttributes(ped, 0, false)   -- Never flee
    pcall(function()
        Citizen.InvokeNative(0x8ACC0506743A8A5C, ped, GetHashKey('ChargeAttack'), 1, -1.0)
    end)
    ...
    -- Disable native 1-shot headshot kills so headshots can be randomized and survive 1 hit
    if type(SetPedSuffersCriticalHits) == 'function' then
        SetPedSuffersCriticalHits(ped, false)
    end
    pcall(Citizen.InvokeNative, 0xEBD45EB69E1D2D07, ped, false)
...
local function DeleteZombieEntity(ped)
    if not ped or not DoesEntityExist(ped) then return end
    pcall(function()
...
RegisterNetEvent('thehunt_zombie:remove',function(id)
    if source~=65535 then return end
    local r=ZC.peds[id]
    local ped = r and (r.corpsePed or r.ped or ZC.entity(r) or (ZC.localPeds and ZC.localPeds[id]))
    if not ped and r and r.net and type(NetworkGetEntityFromNetworkId) == 'function' then
        local np = NetworkGetEntityFromNetworkId(r.net)
        if np and np ~= 0 and DoesEntityExist(np) then ped = np end
    end
    if ped and DoesEntityExist(ped) then
...
CreateThread(function()
    ...
    while true do
        ...
        for id,t in pairs(ZC.cancelled) do if now>t then ZC.cancelled[id]=nil end end
    end
end)
