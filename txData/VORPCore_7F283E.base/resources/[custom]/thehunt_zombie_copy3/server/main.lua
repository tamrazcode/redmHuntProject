local requests,noiseRate,syncRate,busy={},{},{},{}
...
-- Only the victim reports a real native contact against their own player ped.
RegisterNetEvent('thehunt_zombie:attack',function(id,target)
    local src=source; local r=ZS.peds[id]; target=tonumber(target)
    if not r or r.dead or not target or ZS.paused or ZS.immune[target] or r.settings.aggression<=0 then return end
    if src~=target or not ZS.exists(r) then return end
    local ped=GetPlayerPed(target); local now=GetGameTimer()
    if ped==0 or not DoesEntityExist(ped) or GetPlayerRoutingBucket(target)~=r.bucket then return end
    -- Cooldown: server enforces minimum time between hits with slight latency tolerance
    if r.lastAttack and r.lastAttack>0 and now-r.lastAttack<((r.settings.attackCooldown or 500) * 0.8) then return end
    -- Distance validation: generous range to account for server position sync lag and OneSync (0,0,0) desync
    local zCoords = GetEntityCoords(r.entity)
    local pCoords = GetEntityCoords(ped)
    if zCoords and pCoords and (zCoords.x ~= 0.0 or zCoords.y ~= 0.0) then
        local dist = Zombie.distance(zCoords, pCoords)
        if dist > math.max(6.0, (r.settings.attackRange or 2.4) + 3.0) then return end
    end
    r.lastAttack=now; r.attackSequence=(r.attackSequence or 0)+1
    local damage=tonumber(r.settings.damage) or 18
    local weaponKey=r.weapon or (r.settings and r.settings.chosenWeapon)
    if weaponKey and weaponKey~='none' then
        if r.settings and tonumber(r.settings.weaponDamage) then
            damage=tonumber(r.settings.weaponDamage)
        elseif ZombieConfig.WeaponDamage then
            if ZombieConfig.WeaponDamage[weaponKey] then
                damage=ZombieConfig.WeaponDamage[weaponKey]
            else
                for k,dmg in pairs(ZombieConfig.WeaponDamage) do
                    if GetHashKey(k)==weaponKey or tostring(k):upper()==tostring(weaponKey):upper() then
                        damage=dmg; break
                    end
                end
            end
    ...
    TriggerClientEvent('thehunt_zombie:hit',target,id,r.net,r.attackSequence,damage,r.settings.attackRange or 2.4)
end)

RegisterNetEvent('thehunt_zombie:dismountRider',function(target,zombieId)
    local targetId=tonumber(target)
    if not targetId or ZS.immune[targetId] then return end
    TriggerClientEvent('thehunt_zombie:forceDismount',targetId,zombieId)
end)
...
RegisterNetEvent('thehunt_zombie:noise',function(kind,weapon)
    local src=source; local now=GetGameTimer()
    if not ZS.ready or ZS.paused or ZS.immune[src] or not ZombieConfig.Noise[kind] then return end
