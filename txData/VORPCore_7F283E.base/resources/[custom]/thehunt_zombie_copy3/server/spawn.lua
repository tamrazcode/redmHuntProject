...
function ZS.spawn(zone,owner,players)
    ...
    if chosenWalk == 'random' then
        local zombieWalks = { 'MP_Style_drunk', 'MP_Style_Crazy', 'MP_Style_SilentType', 'MP_Style_EasyRider', 'MP_Style_Greenhorn' }
    ...
    local outfits=ZombieModels[model]
    ZS.tickets[id]={id=id,zone=zone,owner=owner,pos=pos,model=model,spawnRadius=z.spawnRadius,
        outfit=outfits[math.random(#outfits)],settings=settings,weapon=chosenWeapon,expires=GetGameTimer()+C.SpawnTimeout,bucket=z.bucket}
...
function ZS.markDead(id)
    ...
    if not r or r.dead or not ZS.exists(r) then return end
    r.dead=true; r.diedAt=GetGameTimer(); r.state='DEAD'; r.target=nil
    ZS.cooldown(r.zone,(ZS.zones[r.zone] or {}).respawn or 120)
    ZS.publish(r)
    TriggerEvent('thehunt_zombie:died',id,r.net,r.zone)
end
RegisterNetEvent('thehunt_zombie:pedDied',function(id)
    local r=ZS.peds[id]
    if r and ZS.exists(r) and NetworkGetEntityOwner(r.entity)==source then ZS.markDead(id) end
end)
function ZS.tick()
    if not ZS.ready then return end
    ...
    for id,r in pairs(ZS.peds) do
        if not ZS.exists(r) then
            -- A streamed/lost entity is not evidence of death. Do not start respawn.
            remove[#remove+1]=id
        else
            ...
            -- Normal streaming cleanup: leave the area and live NPCs are removed.
            -- This is independent from the zone's respawn-after-death cooldown.
            if (r.dead and now-r.diedAt>C.CorpseLifetime) or now-r.lastNear>C.DespawnGrace then
                remove[#remove+1]=id
            end
        end
    end
