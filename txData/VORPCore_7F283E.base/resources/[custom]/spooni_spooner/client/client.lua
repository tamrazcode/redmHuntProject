...
function SpawnSpooni(name, model, x, y, z, pitch, roll, yaw, collisionDisabled, isVisible, lightsIntensity, lightsColour, lightsType)
	...
	if not Config.isRDR and Database[spooni] then
		Database[spooni].isFrozen = true
	end
...
RegisterCommand('spooner_toggle', function()
	TriggerServerEvent('spooner:toggle')
end, false)

if RegisterKeyMapping then
	pcall(RegisterKeyMapping, 'spooner_toggle', 'Spooner: Toggle Menu/Mode', 'keyboard', 'END')
end

function MainSpoonerUpdates()
	if Config.ToggleControl and Config.ToggleControl ~= 0 and IsUsingKeyboard(0) and CheckControls(IsDisabledControlJustPressed, 0, Config.ToggleControl) then
		TriggerServerEvent('spooner:toggle')
	end
...
local function enumerateSpooni()
...
function InitPrompts()
  CreatePromptButton('spooner', "Rotate" .. " -", 'INPUT_SELECT_NEXT_WEAPON')
  CreatePromptButton('spooner', "Rotate" .. " +", 'INPUT_SELECT_PREV_WEAPON')
  CreatePromptButton('spooner', "Up", 'INPUT_FRONTEND_UP')
  CreatePromptButton('spooner', "Down", 'INPUT_FRONTEND_DOWN')
  CreatePromptButton('spooner',"Place", 'INPUT_FRONTEND_ACCEPT', 1500)
  CreatePromptButton('spooner',"Cancel", 'INPUT_FRONTEND_CANCEL', 1500)
end
    ...
    CreateThread(function()
        while true do
            ...
            if hit and #(targetCoord - Origin) <= distance and #(previousCoord - targetCoord) > 0.025 then
                targetCoord = vector3(targetCoord.x,targetCoord.y,targetCoord.z+zValue)
            ...
            if IsPromptCompleted('spooner','INPUT_FRONTEND_CANCEL') then
